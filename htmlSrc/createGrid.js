...
createGrid.prototype = {
	asHtml: function(){
		...
		var innerHTML = Object.keys(grid).map(function (elementName){
			...
			var buttonBody = element['value'] == ('empty') ? 'O' : element['value'];
			return '<'+element['tag']+' '+elementAttributes+'>'+buttonBody+'</'+element['tag']+'>';
		}).join(' ');
		console.log(innerHTML);
		return innerHTML.replace(/<CRNL >undefined<\/CRNL>/g,'<br>');
	},
